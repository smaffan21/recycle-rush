...
public class PlayerScript_Level1 : MonoBehaviour
{
    [SerializeField]
    float speed;
    public int score = 0;
    ...
    {
        if (collision.tag == "Bag" || collision.tag == "Straws") 
        {
            src.clip = scoreUp; 
            src.Play();
        ...
        collision.tag == "Papers" || collision.tag == "Soda1" || collision.tag == "Soda2") 
        {
            src.clip = error; 
            src.Play();
    ...
    {   
        if (score == 8) // next level validity check
        {

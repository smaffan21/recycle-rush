...
public class PlayerScript_Level2 : MonoBehaviour
{
    [SerializeField]
    ...
    public DespawnerScript_Level2 despawnerScript;
    ...
    private void OnTriggerEnter2D(Collider2D collision) // if object hits player, add/subtract 1 score
    {
        ...
        else if (collision.tag == "BlueBottle" || collision.tag == "GreenBottle"||  collision.tag == "Soda1" || collision.tag == "Soda2" || collision.tag == "Bag" || collision.tag == "Straws") 
        {
            src.clip = error; 
            src.Play();
    ...
    {   
        if (score == 10) // next level validity check
        {

...
public class PlayerScript_Level3 : MonoBehaviour
{
    [SerializeField]
    float speed;
    public int score = 0;
    public DespawnerScript_Level3 despawnerScript;
    ...
    private void OnTriggerEnter2D(Collider2D collision) // if object hits player, add/subtract 1 score
    {
        ...
        else if (collision.tag == "BlueBottle" || collision.tag == "GreenBottle"|| collision.tag == "PaperBall" || 
        collision.tag == "Papers" || collision.tag == "Bag" || collision.tag == "Straws") 
        {
            src.clip = error; 
            src.Play();
    ...
    {   
        if (score == 12) // next level validity check
        {
